...
- **Tapback reactions** - Receive 👍❤️😂‼️❓👎 as messages
...
## Update: Zombie Cleanup CONFIRMED WORKING
- Tested: old instance `qkgs` logged "No longer active instance, stopping" and cleaned up
- New instance `3j7w` took over cleanly
- The file-based active instance approach works across hot reloads!

## TODO: Reply Context Needs Work
- Removed reply context injection from message body (user found it cluttery)
- BUT now I can't see what users are quoting without manual DB lookup
- Need to either:
  1. Pass reply context as separate metadata field (not in body)
  2. Or give myself a way to query it when needed
- Currently doing manual sqlite3 lookups which is slow
